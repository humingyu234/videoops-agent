...
public class SecurityConfig implements WebMvcConfigurer {
    ...
    /**
     * 在 Sa-Token 上下文或登录态解析前拒绝歧义凭据。
     *
     * @return 严格凭据入站过滤器注册配置
     */
    @Bean
    public FilterRegistrationBean<StrictHeaderCredentialFilter> strictHeaderCredentialFilterRegistration() {
        FilterRegistrationBean<StrictHeaderCredentialFilter> registration = new FilterRegistrationBean<>();
        registration.setFilter(new StrictHeaderCredentialFilter(securityProperties));
        registration.setName("strictHeaderCredentialFilter");
        ...
        registry.addInterceptor(new SaInterceptor(handler -> {
                ...
                SaRouter
                    ...
                    .check(() -> {
                        ...
                        // 只接受规范 Header 中的 clientid，拒绝 query/form 回退来源。
                        String headerCid = request.getHeader(LoginHelper.CLIENT_KEY);
                        String clientId = getTokenExtra(LoginHelper.CLIENT_KEY);
                        if (StringUtils.isBlank(headerCid) || StringUtils.isBlank(clientId)
                            || !StringUtils.equals(clientId, headerCid)) {
                            throw clientIdMismatchException(StpUtil.getLoginType());
                        }
    ...
    public SaServletFilter getSaServletFilter() {
        String username = requiredActuatorBasicCredential("spring.boot.admin.client.username",
            SpringUtils.getProperty("spring.boot.admin.client.username"));
        String password = requiredActuatorBasicCredential("spring.boot.admin.client.password",
            SpringUtils.getProperty("spring.boot.admin.client.password"));
        return new SaServletFilter()
            ...
            .setError(e -> {
                HttpServletResponse response = ServletUtils.getResponse();
                response.setContentType(SaTokenConsts.CONTENT_TYPE_APPLICATION_JSON);
                return actuatorUnauthorizedResult();
            });
    }

    /**
     * 创建不附带请求凭据的客户端标识不匹配异常。
     *
     * @param loginType 当前登录类型
     * @return 安全的未登录异常
     */
    static NotLoginException clientIdMismatchException(String loginType) {
        return NotLoginException.newInstance(loginType, "-100", "客户端ID与Token不匹配", null);
    }

    /**
     * 创建不暴露 Basic 凭据校验细节的 actuator 鉴权失败响应。
     *
     * @return 固定的未授权响应
     */
    static SaResult actuatorUnauthorizedResult() {
        return SaResult.error("未授权访问").setCode(HttpStatus.UNAUTHORIZED);
    }
    ...
    private void validateClientAccessRules(HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(ipWhitelist)) {
            String clientIp = directPeerIp(request);
    ...
    /**
     * Resolve the direct network peer for an IP allowlist decision.
     *
     * <p>Forwarding headers are attacker-controlled until a trusted reverse-proxy chain is explicitly configured.</p>
     *
     * @param request current HTTP request
     * @return direct peer IP as observed by the servlet container
     */
    static String directPeerIp(HttpServletRequest request) {
        return StringUtils.strip(request.getRemoteAddr(), "[]");
    }

    static String requiredActuatorBasicCredential(String propertyName, String value) {
        if (StringUtils.isBlank(value) || (value.startsWith("${") && value.endsWith("}"))) {
            throw new IllegalStateException("Actuator Basic authentication requires an externally configured " + propertyName);
        }
        return value;
    }
