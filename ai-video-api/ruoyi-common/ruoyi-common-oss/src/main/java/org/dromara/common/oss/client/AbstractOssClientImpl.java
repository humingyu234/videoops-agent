...
public abstract class AbstractOssClientImpl implements OssClient {
    ...
    private PutObjectResult bucketUpload(String bucket, String key, AsyncRequestBody body, Options options) {
        ...
        HandleAsyncResult<PutObjectResponse> result = doCustomUpload(body, builder -> {
            builder.bucket(bucket)
                ...
                .metadata(metadata);
            applyConditionalUpload(builder, options);
    ...
    /** Applies the provider-native atomic create-only guard. */
    protected void applyConditionalUpload(PutObjectRequest.Builder builder, Options options) {
        String ifNoneMatch = options.getIfNoneMatch();
        if (StringUtils.isBlank(ifNoneMatch)) {
            return;
        }
        if ("*".equals(ifNoneMatch) && isAliyunEndpoint()) {
            builder.overrideConfiguration(configuration ->
                configuration.putHeader("x-oss-forbid-overwrite", "true"));
            return;
        }
    ...
    /**
     * Alibaba Cloud OSS inherits the bucket ACL when an object ACL is omitted. Apply the
     * configured canned ACL explicitly so private creator assets remain private even when
     * they share a public-read development bucket. Other S3-compatible providers retain
     * their existing request behavior because several of them reject canned ACL headers.
     *
     * @param builder upload request builder
     */
    protected void applyConfiguredObjectAcl(PutObjectRequest.Builder builder) {
        if (!isAliyunEndpoint()) {
            return;
        }
        var accessControl = config.accessControlPolicyConfig();
        if (accessControl.enabled()) {
            builder.acl(accessControl.accessPolicy().getObjectCannedACL());
        }
    }

    private boolean isAliyunEndpoint() {
        return config.endpoint()
            .map(endpoint -> {
                ...
                try {
                    ...
                    return host != null && ("aliyuncs.com".equalsIgnoreCase(host)
    ...
    @Override
    public InputStream doCustomBufferedDownload(Consumer<GetObjectRequest.Builder> getObjectRequestBuilderConsumer,
                                                Duration timeout) {
        if (timeout == null || timeout.isZero() || timeout.isNegative()
            || timeout.compareTo(Duration.ofSeconds(30)) > 0) {
            throw S3StorageException.form("download timeout must be between 1ms and 30s");
        }
        Path directory = null;
        Path target = null;
        CompletableFuture<GetObjectResponse> download = null;
        try {
            directory = Files.createTempDirectory("ruoyi-oss-download-");
            target = directory.resolve("content");
            GetObjectRequest.Builder request = GetObjectRequest.builder();
            getObjectRequestBuilderConsumer.accept(request);
            Duration attemptTimeout = timeout.compareTo(Duration.ofSeconds(20)) > 0
                ? Duration.ofSeconds(20) : timeout;
            request.overrideConfiguration(options -> options
                .apiCallAttemptTimeout(attemptTimeout)
                .apiCallTimeout(timeout));
            download = s3AsyncClient.getObject(request.build(), target);
            try {
                download.get(timeout.toMillis(), TimeUnit.MILLISECONDS);
            } catch (InterruptedException interrupted) {
                Thread.currentThread().interrupt();
                throw interrupted;
            }
            return new DeleteOnCloseInputStream(Files.newInputStream(target), target, directory);
        } catch (Exception e) {
            if (download != null) {
                download.cancel(true);
            }
            deleteTemporaryDownload(target, directory);
            throw toStorageException(e);
        }
    }

    private static void deleteTemporaryDownload(Path target, Path directory) {
        try {
            if (target != null) {
            ...
            // Cleanup is best effort after a failed or closed download.
        }
    }
    ...
    private static final class DeleteOnCloseInputStream extends FilterInputStream {
        ...
        private DeleteOnCloseInputStream(InputStream input, Path target, Path directory) {
            super(input);
            this.target = target;
