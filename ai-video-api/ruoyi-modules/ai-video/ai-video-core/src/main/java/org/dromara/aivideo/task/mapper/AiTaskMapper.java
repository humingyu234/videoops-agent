...
public interface AiTaskMapper extends BaseMapperPlus<AiTask, AiTask> {
    ...
    @Select("""
        SELECT COUNT(*)
        FROM av_workflow_order
        WHERE order_id = #{orderId}
          AND tenant_id = #{tenantId}
          AND owner_user_id = #{ownerUserId}
          AND workspace_id = #{workspaceId}
        """)
    int countOwnedWorkflowOrder(@Param("orderId") Long orderId, @Param("tenantId") Long tenantId,
                                @Param("ownerUserId") Long ownerUserId, @Param("workspaceId") String workspaceId);
}
