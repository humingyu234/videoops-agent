...
- 当前阶段所必需的本机开发/测试前置默认已获负责人授权：可以从明确命名的旧项目本机 dev 配置或 Git 忽略的 DPAPI 载体临时复用凭据，仅用于创建和操作本项目隔离资源及最小权限账号。凭据只能在受控进程内使用，不得回显、写入新仓、进入命令参数/日志/持久环境；不得因此读取或修改公司业务数据。本授权不包含生产环境、破坏性操作、外部发布或付费 Provider 调用。
- 跨数据库、存储或 Provider 等真实边界前，只做一次最小能力预检：精确目标、载体字段、登录身份、所需授权和一条只读证明。必须区分“负责人授权、可用凭据、账号权限、现场证明”，不得根据文件名或历史文档猜测；缺项时一次性报告最小输入，同时继续不依赖该输入的安全工作。
- 秘密载体只允许检查路径、ACL 和字段名等元数据；需要解密时必须在隔离子进程内抑制输出。禁止用 `Get-Content` 或递归内容搜索排查秘密文件；秘密值不得作为 PowerShell 函数、cmdlet 或异常消息参数。一次性凭据操作禁止自建 YAML 解析器、短名函数/别名和大型诊断脚本，只允许精确字段读取、固定状态码与 `finally` 清理。若秘密意外进入输出，立即停止并如实报告；是否轮换由负责人按实际暴露范围决定。
