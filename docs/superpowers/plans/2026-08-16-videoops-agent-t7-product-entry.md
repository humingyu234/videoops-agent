...
- 公开 GitHub 目标固定为 `humingyu234/videoops-agent`；现场 `origin/main` 为 `3740c5e38cc6f19f466c34b16fa1cbf5561966a7`。本地 T7 checkpoint 不由当前线程 push，远端更新由监督线程执行；生产部署和签名仍为 `NOT_RUN`。
...
- 四场景、最终 MP4、README/许可证、测试与证据匹配同一 commit；真实全链使用项目 DB14、OSS 和已授权 Provider；仓库公开可见性为 `PUBLIC`，生产发布仍标为 `NOT_RUN`。

## 停止条件

真实 Provider 已接受但结果未知时不得换键重提；需要第二次付费生成、会破坏隔离数据或无法安全取得凭据时，只冻结对应高风险动作，其余安全施工继续。远端变更由监督线程执行；T7 完成后不进入 T8。
