...
以下未带工程前缀的路径和命令均相对于 `<repo>/ai-video-api`；项目级 `docs/`、`AGENTS.md` 与 `RULES.md` 从仓库根目录解析。
...
2. 当前业务模块下符合上述硬约束的最近似实现。
3. 当前仓库公共能力模块中的统一约定。
4. generator 模板。
5. 通用 Spring / MyBatis-Plus 默认习惯。

项目硬约束不得被遗留代码覆盖；仅在硬约束未规定实现细节时，才优先相信当前仓库中符合约束的真实代码。
...
- `domain/Entity.java`
- `dto/EntityDTO.java`（仅限 `ai-video-core` 中稳定跨模块 Service 数据契约）
...
- 业务聚合只能使用上述 RuoYi 业务包；端侧 HTTP 模块使用 `domain.bo`、`domain.vo`、`controller`，共享核心模块没有 HTTP（超文本传输协议）入口时省略三者。`dto` 与 `domain`、`mapper`、`service` 平级，只承载数据契约。Service 接口使用 `I...Service`，实现类使用 `...ServiceImpl` 并放在 `service.impl`。
- Entity 保持贫血持久化对象：字段、表映射和简单内聚判断可以留在 Entity；事务、状态流转、归属、幂等、额度、外部调用和跨 Mapper 编排必须在 Service。
- 禁止把 `application`、`application.impl`、`port`、`adapter`、`command`、`model`、`aggregate`、`repository` 或 CQRS（命令查询职责分离）作为业务分层，禁止以 DDD（领域驱动设计）、Clean Architecture（整洁架构）或 Hexagonal Architecture（六边形架构）替换 RuoYi 分层。
- 端侧请求语义使用 BO，响应语义使用 VO；AI 视频业务专属的稳定跨模块 Service 契约使用 `ai-video-core` 对应业务聚合平级 `dto` 包中的 `*DTO`，禁止迁入全局 `ruoyi-api`。供应商原始请求／响应只留在 `ai-video-infra` 的直接 `client`／`provider` 边界。`config`、`security`、`event`、`listener`、`constant`、`enums`、`properties`、`utils`、`client`、`provider` 不能形成第二套业务 Service。
- 安全／框架例外必须先在 `docs/DOMAIN_MODEL.md` 与模块规格记录原因、最小范围、替代控制和回归条件，并取得项目负责人明确确认。
