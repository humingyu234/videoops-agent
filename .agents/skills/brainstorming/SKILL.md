---
name: brainstorming
description: "用于用户明确要求通过互动头脑风暴探索尚未批准的新功能，或明确要求把讨论收敛成设计规格时。不要用于普通方案比较、执行已有计划、范围清楚的实现、普通修复、只读审计或进度更新。"
license: MIT
metadata:
  hermes:
...
通过自然的协作对话帮助用户探索和收敛设计。只有命中上面的触发条件才使用本 skill；项目当前施工由 `docs/EXECUTION.md` 及其指向的 runbook 驱动。默认先完成对话与决策摘要，只有用户明确要求把结果写入仓库时才创建规格文件。

首先了解当前项目的上下文，然后逐一提问来完善想法。一旦你理解了要构建的内容，就展示设计方案并获得用户批准。

<HARD-GATE>
在你展示设计方案并获得用户批准之前，不要编写实现代码、搭建项目或采取实现行动。只做头脑风暴时不写文件；写规格和提交 Git 都需要当前任务已经明确授权。
</HARD-GATE>

## 反模式："这个太简单了，不需要设计"

一旦明确选择本 skill，就不要因为功能看似简单而跳过关键假设；设计可以只有几句话。范围已经清楚的实现、普通修复和配置小改不应触发本 skill，也不需要补做完整设计流程。
...
6. **按需编写设计文档** — 仅在用户要求持久化规格时保存到 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`
7. **规格自检** — 快速内联检查占位符、矛盾、模糊性、范围（详见下方）
8. **用户审查书面规格** — 仅在创建了规格文件时请用户审查
9. **按需过渡到计划** — 仅在用户要求实现计划时调用 writing-plans skill
...
digraph brainstorming {
    ...
    "用户要求写入规格?" [shape=diamond];
    "返回决策摘要" [shape=doublecircle];
    "编写设计文档" [shape=box];
    "规格自检\n（内联修复）" [shape=box];
    "用户审查规格?" [shape=diamond];
    "用户要求实现计划?" [shape=diamond];
    "结束于已批准规格" [shape=doublecircle];
    ...
    "用户批准设计?" -> "用户要求写入规格?" [label="是"];
    "用户要求写入规格?" -> "返回决策摘要" [label="否"];
    "用户要求写入规格?" -> "编写设计文档" [label="是"];
    "编写设计文档" -> "规格自检\n（内联修复）";
    "规格自检\n（内联修复）" -> "用户审查规格?";
    "用户审查规格?" -> "编写设计文档" [label="要求修改"];
    "用户审查规格?" -> "用户要求实现计划?" [label="批准"];
    "用户要求实现计划?" -> "结束于已批准规格" [label="否"];
    "用户要求实现计划?" -> "调用 writing-plans 技能" [label="是"];
}
```

终止状态取决于用户授权：只探索时返回决策摘要；要求持久化时结束于已批准规格；明确要求实现计划时才调用 writing-plans。不要自动进入实现或加载无关 Skill。
...
- 只有当前任务明确要求持久化规格时，才将验证通过的设计写入 `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`
  - （用户对规格位置的偏好优先于此默认值）
- 未要求写文件时，只在对话中给出决策摘要。
- 是否提交 Git 服从当前任务授权和项目施工协议，不由本 skill 自动决定。
...
> "规格已写入 `<path>`。请审查一下；如果需要修改，或希望我继续创建实现计划，请告诉我。"

等待用户回复。如果他们要求修改，做出修改并重新运行规格自检。只有在用户批准后才继续。

**实现：**

- 只有用户要求进入实现规划时，才调用 writing-plans skill 创建详细计划。
- 用户只要求探索或评审时，到批准后的决策摘要为止。
