...
本 skill 可以从仓库内任意工作目录触发。执行相对路径命令前，先定位 Git 仓库根目录，再把工作目录切换到 `<repo>/ai-video-ui/ai-video-platform-ui`。下文未带前缀的 `src/`、`gen/`、`package.json` 和项目命令都相对于该前端工程；跨工程路径则从仓库根目录解析。
...
5. 需要理解 Vue 版生成器意图时，按需读取同级 [RuoYi 前端 reference](../ruoyi-plus-ai-coding/references/frontend.md)；只吸收字段、权限、任务分型和自检原则，不依赖仓库外的机器目录。
6. 新增标准页面前，可以对照仓库内后端工程 `<repo>/ai-video-api/ruoyi-modules/ruoyi-gen/src/main/resources/fm` 确认字段、权限和导出能力，但输出和 `gen/` 模板必须保持 React 项目风格。
...
4. 同级 RuoYi 前端 reference 或用户明确提供的 Vue 参考项目中的任务分型和工作流。
