...
Resolve the repository root and target frontend before running commands:

- User web: `<repo>/ai-video-ui/ai-video-webapp`; this package owns the local `@ant-design/cli` dev dependency.
- Platform UI: `<repo>/ai-video-ui/ai-video-platform-ui`; read its `package.json` for the exact Ant Design version and do not assume the CLI is installed there.

Never run `npx antd` from the repository root. For user-web work, change to the webapp directory first. For platform metadata queries, run the webapp's installed CLI with an explicit `--version` matching platform-ui. Run source scans such as `lint` or `usage` against the actual target package only when that package has a usable CLI; otherwise report that the Ant Design-specific scan was not run and use the target package's project-native checks plus official documentation.
...
2. **Match the target package version** — Both frontend packages currently use antd v6, but their exact versions can differ. Read the target `package.json`; rely on auto-detection only after changing to that package, otherwise pass `--version` explicitly.
